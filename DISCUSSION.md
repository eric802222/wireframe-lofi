...
- [2026-07-09] **density 總開關定案（收斂 line 67 長年待決；實測回饋「mockup 間距太窄」）** → `theme.base.density: compact|comfortable`。這正是「間距語義 scale 換 theme 一字不動」承諾的兌現：sm/md/lg/xl 語義名與 YAML 全不動，comfortable 只換 `:root` 刻度值（.25/.5/1/2 → .5/1/1.5/2.5rem + page-pad 24px）。wireframe 密（討論用資訊密度）、mockup 鬆（產品呼吸感）——密度也是保真度的函數。
