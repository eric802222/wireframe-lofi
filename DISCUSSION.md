...
- [2026-07-09] **P7 phase 2b：mockup 元件皮 + theme 綁 `name:` + gutter 外置**（實測回饋二輪「還不夠像」+「note 應該在 viewport 外面」）：
...
- [2026-07-09] **P7 phase 2c：基底皮從硬編改宣告（使用者抓漂移：「樣式應該都是用 mockup 套入的而不是寫死的」）** → phase 2a/2b 把 `_MOCKUP_BASE_CSS` 硬編進編譯器，正是 P7 要消滅的「CSS-driven、卡在工具裡」；撤除，改 theme YAML 兩段宣告：
  - **`base:`**（全域基底，值全受控 enum）：`font: product|wireframe`、`chrome: card|flat`、`radius: <scale>`、`link-marker: hide|show`、`brand: <hex>`（+選配 `brand-soft`，未給用 `color-mix` 自動衍生）。**brand 是 theme 檔唯一允許的物理值**——theme 即物理綁定層，產品色住這裡（tone 三歸宿兌現處）。不寫 `base:` = 純 bindings（向後相容）。
  - **`bindings:` 擴到內建元件 role**：`button` / `button-link` / `input` / `select` / `status(.muted/.strong/.badge)` / `box` 映射 wf-* 契約 selector——同一套 bindable 詞彙換元件皮，零新語彙。
  - **不變式**：工具不硬編任何 mockup 長相；標註面字體守衛（gutter 維持 mono）屬機制非樣式，留在編譯器。
- [2026-07-09] ~~density 總開關 `base.density: compact|comfortable`~~ → **當日被 phase 2d 取代**（preset 表又把值寫回工具——使用者二度抓漂移）。「密度也是保真度的函數」的洞察保留，實作改走 tokens 值層。
- [2026-07-09] **P7 phase 2d：theme 開 `tokens:` 值層（終局形）——「工具只認名字，值全歸 theme」**（使用者抓漂移：「這樣又寫回 hardcode 了，應該讓 mockup 細顆粒設定、之後交付給 FE，而不是一直改工具」）：
  - **`tokens:` 段**：Tier-1 design token 值層——`font.body`、`space.sm-xl`、`radius.*`、`shadow.*`、`color.*`（brand/surface 系/ink 系/line 系/page）、`page.pad`。名字驗證 fail-fast、**值透傳**（物理層本就收原始值：hex/rem/stack）。這段 YAML 就是可直接交付 FE 的 design tokens 檔。
  - **bindings 語義名全部改解析成 `var(--wf-*, fallback)`**：內建值降級為 var fallback（可攜地板），tokens 一蓋全站變。工具裡不再存在任何「theme 才有的值」。
  - **`base:` 收縮成純模式開關**：只剩 `chrome: card|flat`、`link-marker: hide|show`（非值、是版面架構/顯隱模式）；font/brand/radius/density preset 全撤，值歸 tokens。
  - **分層總結**：`tokens:` = 值（FE handoff）；`bindings:` = 語義身份 → 語義值（component spec）；`base:` = 模式開關。與 DTCG 對齊路徑：tokens 段一對一映射 `$value`，未來 `--emit tokens` 可直接輸出。
- [2026-07-09] **P7 phase 2e：production-ready 收尾**（實測回饋「mockup 繪製到更 production ready」）：
  - **clean 基底 token 化**：box/btn/input/select/badge 框線、hr、hint/heading 文字色全改讀 `var(--wf-line/-subtle/--wf-ink/-soft, <現值>)`——fallback=現值故 wireframe 零變化；theme tokens 一蓋，全站框線/墨色跟著變。style 層讀 tokens 是分層的正解（值不進工具、也不進 style，style 只引用名字）。
  - **綁定優先序 specificity 化**：語義身份（role/name 三疊 selector）> 元件皮（element）> 基底——「這顆按鈕」贏過「所有按鈕」（oracle-entry 實心 CTA vs button-link outline 並存的機制）。
  - **token 表補 `font.size/h1/h2/h3`**（標題階層是產品感要件）。
  - **品牌色紀律**（theme 作者面 pattern，非工具規則）：`button-link` 退 outline+brand 字、實心 brand 只留真 CTA——色彩省著用才像產品。
