...
瀏覽器開 debug 產物 → 工具列（桌面右上、手機底部）切「**模式:註記**」→ **點任一元素**寫修改建議（存 localStorage、reload 存活）
→「**匯出**」複製清單貼回給 LLM 一次改 YAML。「模式:瀏覽」時點擊照常跳轉（可走動線）。

註記模式會顯示**註記清單**：依來源檔分組，目前頁面來源排最前面；每則顯示路徑、建議及元素快照，
點選即可直接編輯或刪除，不必回原元素。手機點「註記 N」開啟底部抽屜，按「收合」或 Escape 關閉。
清單底部可儲存**整頁註記**，以來源檔為單位（同檔 routes 共用），匯出為 `- (整頁) → 建議`。
既有 localStorage 記錄格式與來源定位不變，編輯、刪除與匯出即時同步；未儲存的整頁草稿可在本次開啟中切頁保留。
