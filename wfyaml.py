...
"""
import sys, os, re, html, gzip, json, yaml
...
def _compile_page(doc, provider, basedir, ctx=None, cur_label=None, all_labels=None, debug=False):
    ...
    css = _BASE_CSS + CSS_EXTRA + (DEBUG_CSS if debug else '') + _width_css('.wf-root', w, h, notes)
...
def bundle(files, debug=False, title='prototype'):
    ...
    css = _BASE_CSS + CSS_EXTRA + BUNDLE_CSS + (DEBUG_CSS if debug else '') + ''.join(overrides)
    tail = ('<script>' + DEBUG_JS + '</script>' if debug else '') + '</body></html>'
...
def compile_yaml(src, basedir='.', base='', debug=False):
    """向後相容：回傳第一份（無路由時即唯一份）。"""
    return compile_all(src, basedir, base, debug)[0][1]


def main():
    debug = '--debug' in sys.argv
    do_bundle = '--bundle' in sys.argv
    out_i = sys.argv.index('-o') if '-o' in sys.argv else -1
    out_path = sys.argv[out_i + 1] if out_i >= 0 else None
    args = [a for a in sys.argv[1:]
            if a not in ('--debug', '--bundle', '-o', out_path)]
    if not args:
        print("usage: wfyaml.py [--debug] [--bundle [-o out.html]] <file.wf.yaml> [...]", file=sys.stderr)
        sys.exit(1)
    if do_bundle:
        out = out_path or os.path.join(os.path.dirname(args[0]) or '.',
                                       'prototype' + ('.debug' if debug else '') + '.html')
        open(out, 'w').write(bundle(args, debug=debug))
        print(f"  bundled: {out} ({len(args)} 檔)")
        return
    for path in args:
        src = open(path).read()
        ...
        for rid, htmlout in compile_all(src, basedir, base, debug=debug):
