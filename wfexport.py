#!/usr/bin/env python3
"""匯出／匯入：把 theme 的 tokens 與 kit 的 props/states 接上工具鏈外的世界。

    python3 wfexport.py tokens <theme.yaml> [--format json|css|ts] [--kit <kit.yaml>]
    python3 wfexport.py types  <kit.yaml>   [--format ts|json] [--types-map <map.yaml>]
    python3 wfexport.py import <tokens.json>                         # DTCG → theme 書寫形式

設計立場：
- **tokens** 輸出對齊 DTCG（designtokens.org）標準，可直接餵 Style Dictionary / Terrazzo。
  書寫層（theme.yaml）維持好寫好讀，標準只出現在輸出層。
- **import** 是同一張對照表反過來走：外部工具的 token 進來，仍還原成一行式的書寫形式。
- **types** 輸出的是「契約」而非實作：接上資料與行為是 RD 那一層的事。
- 獨立模組，不改動 wfyaml.py 的任何行為；渲染與 lint 完全不受影響。
"""
import json
import yaml
import re
import sys

import wfyaml as wf


# ── props 型別（交付 RD 的是契約，不是實作）─────────────────────────────────
# 刻意只給描述「值長什麼樣」的型別；沒有 shape/物件/泛型，那些屬於應用層。
_PROP_TYPES = {'text', 'number', 'boolean', 'date', 'time', 'url'}
_PROP_TS = {'text': 'string', 'number': 'number', 'boolean': 'boolean',
            'date': 'string', 'time': 'string', 'url': 'string'}


def _ts_name(kit_name):
    """kebab-case 型別名 → PascalCase。"""
    return ''.join(part.capitalize() for part in str(kit_name).split('-'))


def _kit_types_ts(components):
    """kit → TS 型別宣告。props 未標型別時視為 text（string）。"""
    lines = ['// generated by wfexport.py types --format ts, do not edit',
             '// 這是契約，不是實作：接上資料與行為是 RD 那一層的事。', '']
    for name in sorted(components):
        spec = components[name]
        ptypes = spec.get('prop-types') or {}
        fields = []
        for prop in spec.get('props', []):
            declared = ptypes.get(prop, 'text')
            if isinstance(declared, list):
                ts = ' | '.join(json.dumps(v, ensure_ascii=False) for v in declared)
            else:
                ts = _PROP_TS[declared]
            fields.append(f'  {prop}: {ts};')
        states = spec.get('states') or []
        if states:
            union = ' | '.join(json.dumps(v) for v in states)
            fields.append(f'  state?: {union};')
        if 'to' not in spec.get('props', []):        # kit 自己宣告了 to 就不重複
            fields.append('  to?: string;            // 動線目標（畫面 id）')
        lines.append(f'export type {_ts_name(name)}Props = {{')
        lines += fields
        lines.append('};')
        lines.append('')
    index = ', '.join(f"'{n}': {_ts_name(n)}Props" for n in sorted(components))
    lines.append(f'export type KitProps = {{ {index} }};')
    lines.append('')
    return '\n'.join(lines)


def _kit_prop_types(map_path):
    """讀型別對照（選配的 sidecar 檔）：`元件名: {prop: 型別}`。

        # kit/prop-types.yaml
        stamp-card: { visits: number, tone: [info, warn] }

    放在 kit 之外是刻意的：wfyaml 的 kit 只認名稱 list，
    本模組不改動它的驗證與渲染。未標註者一律視為 text。
    """
    if not map_path:
        return {}
    raw = wf._read_yaml(map_path) or {}
    if not isinstance(raw, dict):
        raise ValueError('prop-types 檔頂層必須是 {元件名: {prop: 型別}}')
    for name, declared in raw.items():
        if not isinstance(declared, dict):
            raise ValueError(f'{name}：必須是 {{prop: 型別}} dict')
        for prop, ptype in declared.items():
            if isinstance(ptype, list):
                if not ptype or any(not isinstance(v, str) for v in ptype) or len(ptype) != len(set(ptype)):
                    raise ValueError(f'{name}.{prop}：enum 必須是不重複的字串 list')
            elif ptype not in _PROP_TYPES:
                raise ValueError(
                    f'{name}.{prop}：型別只接 {sorted(_PROP_TYPES)} 或 enum list（收到 {ptype!r}）')
    return raw


def _types_export(kit_path, fmt, map_path=None):
    """`types`：kit 的 props / states → 型別宣告。"""
    wf._load_kit(kit_path, explicit=True)
    for cname, declared in _kit_prop_types(map_path).items():
        if cname not in wf._KIT_COMPONENTS:
            raise ValueError(f'prop-types 指到 kit 沒有的元件 `{cname}`')
        unknown = set(declared) - set(wf._KIT_COMPONENTS[cname].get('props', []))
        if unknown:
            raise ValueError(f'{cname}：prop-types 指到不存在的 props {sorted(unknown)}')
        wf._KIT_COMPONENTS[cname]['prop-types'] = declared
    if not wf._KIT_COMPONENTS:
        raise ValueError(f'kit 沒有任何元件：{kit_path}')
    if fmt == 'ts':
        print(_kit_types_ts(wf._KIT_COMPONENTS))
        return
    out = {}
    for name, spec in wf._KIT_COMPONENTS.items():
        ptypes = spec.get('prop-types') or {}
        out[name] = {'props': {p: ptypes.get(p, 'text') for p in spec.get('props', [])},
                     'states': spec.get('states') or [],
                     'of': spec.get('of')}
    print(json.dumps(out, ensure_ascii=False, indent=2))


# ── tokens 匯出：DTCG（Design Tokens Community Group）──────────────────────
# 書寫層（theme.yaml）維持好寫好讀，語義層對齊標準，輸出層產合規 .tokens.json。
# 標準：https://www.designtokens.org/  別名語法 {family.name} 與本工具現行寫法一致。
_DTCG_WEIGHT_ALIASES = {
    'thin', 'hairline', 'extra-light', 'ultra-light', 'light', 'normal', 'regular', 'book',
    'medium', 'semi-bold', 'demi-bold', 'bold', 'extra-bold', 'ultra-bold', 'black', 'heavy',
    'extra-black', 'ultra-black'}


def _dtcg_dimension(text):
    """'14px' / '1.5rem' → {'value': 14, 'unit': 'px'}；不合則 None。"""
    m = re.fullmatch(r'\s*(-?\d+(?:\.\d+)?)(px|rem)\s*', str(text))
    if not m:
        return None
    num = float(m.group(1))
    return {'value': int(num) if num == int(num) else num, 'unit': m.group(2)}


def _dtcg_duration(text):
    """'150ms' / '1.5s' → {'value': 150, 'unit': 'ms'}；不合則 None。"""
    m = re.fullmatch(r'\s*(-?\d+(?:\.\d+)?)(ms|s)\s*', str(text))
    if not m:
        return None
    num = float(m.group(1))
    return {'value': int(num) if num == int(num) else num, 'unit': m.group(2)}


# ── 複合型（DTCG §9）：border / shadow / strokeStyle ────────────────────────
# 書寫層仍是一行 CSS 值（好寫好讀），輸出層才拆成標準的結構化 $value。
_STROKE_STYLES = ('solid', 'dashed', 'dotted', 'double', 'groove', 'ridge', 'outset', 'inset')


def _dtcg_color(text):
    """色值：#hex 或 rgb()/rgba()/hsl() 原樣（穩定版允許字串）。不像色值則 None。"""
    text = text.strip()
    if re.fullmatch(r'#[0-9A-Fa-f]{3,8}', text) or re.match(r'(rgba?|hsla?)\(', text):
        return text
    return None


def _split_css(text):
    """依空白切 CSS 值，但不切開 rgb(...) 這類括號內的逗號與空白。"""
    parts, depth, cur = [], 0, ''
    for ch in str(text):
        if ch == '(':
            depth += 1
        elif ch == ')':
            depth -= 1
        if ch.isspace() and depth == 0:
            if cur:
                parts.append(cur)
                cur = ''
            continue
        cur += ch
    if cur:
        parts.append(cur)
    return parts


def _dtcg_border(text):
    """'2px solid #2B2A33' → DTCG border；順序不限，三者齊全才算。"""
    width = style = color = None
    for part in _split_css(text):
        dim = _dtcg_dimension(part)
        if dim and width is None:
            width = dim
            continue
        if part in _STROKE_STYLES and style is None:
            style = part
            continue
        col = _dtcg_color(part)
        if col and color is None:
            color = col
            continue
        return None
    if width and style and color:
        return {'width': width, 'style': style, 'color': color}
    return None


def _dtcg_shadow(text):
    """'0 4px 0 #2B2A33'（x y blur [spread] color，可帶 inset）→ DTCG shadow。"""
    parts = _split_css(text)
    inset = False
    if 'inset' in parts:
        inset = True
        parts = [p for p in parts if p != 'inset']
    color = None
    for part in list(parts):
        col = _dtcg_color(part)
        if col:
            color = col
            parts.remove(part)
            break
    dims = []
    for part in parts:
        dim = _dtcg_dimension(part) or (_dtcg_dimension(part + 'px')
                                        if re.fullmatch(r'-?\d+(?:\.\d+)?', part) else None)
        if not dim:
            return None
        dims.append(dim)
    if color is None or not 2 <= len(dims) <= 4:
        return None
    zero = {'value': 0, 'unit': 'px'}
    out = {'color': color, 'offsetX': dims[0], 'offsetY': dims[1],
           'blur': dims[2] if len(dims) > 2 else zero,
           'spread': dims[3] if len(dims) > 3 else zero}
    if inset:
        out['inset'] = True
    return out


def _dtcg_stroke_style(text):
    """'4 10' 或 '4px 10px' → DTCG strokeStyle（dashArray）；關鍵字則原樣。"""
    text = str(text).strip()
    if text in _STROKE_STYLES:
        return text
    dims = []
    for part in _split_css(text.replace(',', ' ')):
        dim = _dtcg_dimension(part) or (_dtcg_dimension(part + 'px')
                                        if re.fullmatch(r'-?\d+(?:\.\d+)?', part) else None)
        if not dim:
            return None
        dims.append(dim)
    if len(dims) < 2:
        return None
    return {'dashArray': dims, 'lineCap': 'butt'}


def _dtcg_composite(family, name, text):
    """家族語義決定複合型：shadow / border / stroke.dash。"""
    if family == 'shadow':
        got = _dtcg_shadow(text)
        return ('shadow', got) if got else None
    if family == 'border':
        got = _dtcg_border(text)
        return ('border', got) if got else None
    if family == 'stroke' and 'dash' in str(name):
        got = _dtcg_stroke_style(text)
        return ('strokeStyle', got) if got else None
    return None


_TYPOGRAPHY_SUBS = ('fontFamily', 'fontSize', 'fontWeight', 'lineHeight', 'letterSpacing')


def _dtcg_typography(value):
    """typography 複合值：子值原樣帶出（含 {ref} 別名，DTCG 允許子值是參照）。"""
    if not isinstance(value, dict) or not value:
        return None
    unknown = set(value) - set(_TYPOGRAPHY_SUBS)
    if unknown:
        return None
    return {k: (v if isinstance(v, str) else v) for k, v in value.items()}


def _dtcg_gradient(value):
    """gradient 複合值：DTCG 是一組 {color, position}；方向不在 DTCG 的定義裡(見其 issue #101),
    所以我們的 angle 不往外送,匯出的是規格內的部分。"""
    stops = value.get('$value') if isinstance(value, dict) else value
    if not isinstance(stops, list) or not stops:
        return None
    out = []
    for stop in stops:
        if not isinstance(stop, dict) or 'color' not in stop:
            return None
        item = {'color': stop['color']}
        if 'position' in stop:
            item['position'] = stop['position']
        out.append(item)
    return out


def _dtcg_token(family, name, value):
    """單一 token → (dtcg_type, dtcg_value)；無法對應標準型別時回 (None, 原因)。

    優先序：別名 → 家族語義（複合型、color、font）→ 值的形狀。
    刻意不猜：對不上就不匯出並說明原因，而不是塞一個不合規的型別。
    """
    if family == 'typography':
        got = _dtcg_typography(value if not isinstance(value, dict) or '$value' not in value else value['$value'])
        return ('typography', got) if got else (None, 'typography 子值需為 DTCG 的 fontFamily/fontSize/fontWeight/lineHeight/letterSpacing')
    if family == 'gradient':
        got = _dtcg_gradient(value)
        return ('gradient', got) if got else (None, 'gradient 需為一組 {color, position}')

    if isinstance(value, (list, tuple)):
        nums = list(value)
        if len(nums) == 4 and all(isinstance(n, (int, float)) for n in nums):
            if not (0 <= nums[0] <= 1 and 0 <= nums[2] <= 1):
                return None, 'cubicBezier 的 x 座標必須落在 [0, 1]'
            return 'cubicBezier', [float(n) for n in nums]
        if all(isinstance(n, str) for n in nums):
            return 'fontFamily', list(nums)
        return None, '陣列只支援 cubicBezier（四個數字）或 fontFamily（字串陣列）'

    text = str(value).strip()
    if text.startswith('{') and text.endswith('}'):
        return 'alias', text                      # 型別由被參照的 token 決定（DTCG §5.2.2）

    composite = _dtcg_composite(family, name, text)
    if composite:
        return composite

    if family == 'color':
        return 'color', text
    if family == 'font':
        if name in ('body', 'family', 'mono', 'serif', 'sans'):
            return 'fontFamily', text
        if name in ('weight', 'bold', 'regular'):
            if re.fullmatch(r'\d+', text):
                w = int(text)
                if not 1 <= w <= 1000:
                    return None, 'fontWeight 必須在 1–1000'
                return 'fontWeight', w
            if text in _DTCG_WEIGHT_ALIASES:
                return 'fontWeight', text
            return None, f'fontWeight 不接受 {text!r}（1–1000 或標準別名）'

    dim = _dtcg_dimension(text)
    if dim:
        return 'dimension', dim
    dur = _dtcg_duration(text)
    if dur:
        return 'duration', dur
    if re.fullmatch(r'-?\d+(?:\.\d+)?', text):
        num = float(text)
        return 'number', int(num) if num == int(num) else num
    if re.fullmatch(r'#[0-9A-Fa-f]{3,8}', text):
        return 'color', text

    if re.fullmatch(r'-?\d+(?:\.\d+)?(em|%|vh|vw|ch)', text):
        return None, f'DTCG dimension 只支援 px / rem（收到 {text!r}）'
    return None, f'無對應的 DTCG 型別（{text!r}）'


def _token_raw(entry):
    """取 token 原值：已是 DTCG 形狀（含 $value）就取 $value，否則原樣。"""
    if isinstance(entry, dict) and '$value' in entry:
        return entry['$value']
    return entry


def _tokens_to_dtcg(tokens):
    """theme tokens → (DTCG dict, 略過清單)。preset 不是 token，不匯出。"""
    out, skipped = {}, []
    for family, entries in (tokens or {}).items():
        if family == 'preset' or not isinstance(entries, dict):
            continue
        group, types = {}, set()
        for name, entry in entries.items():
            kind, val = _dtcg_token(family, name, _token_raw(entry))
            if kind is None:
                skipped.append((f'{family}.{name}', val))
                continue
            if kind == 'alias':
                group[str(name)] = {'$value': val}
                continue
            group[str(name)] = {'$type': kind, '$value': val}
            if kind == 'gradient':
                raw = entries[name]
                angle = raw.get('angle') if isinstance(raw, dict) else None
                if angle is not None:      # 方向不在 DTCG 的 gradient 定義裡（見其 issue #101）→ $extensions
                    group[str(name)]['$extensions'] = {'space.ggmant.wireframe-lofi': {'angle': str(angle)}}
            types.add(kind)
        if not group:
            continue
        if len(types) == 1:                       # 同型家族：$type 提到群組層（DTCG §6.3）
            only = types.pop()
            for tok in group.values():
                tok.pop('$type', None)
            out[family] = {'$type': only, **group}
        else:
            out[family] = group
    return out, skipped


def _token_css_value(value):
    """token 值 → CSS 值。陣列依形狀轉 cubic-bezier() 或字型後備串。"""
    if isinstance(value, (list, tuple)):
        nums = list(value)
        if len(nums) == 4 and all(isinstance(n, (int, float)) for n in nums):
            return 'cubic-bezier(' + ', '.join(str(n) for n in nums) + ')'
        return ', '.join(str(n) for n in nums)
    return wf._resolve_value(str(value))


def _tokens_as_css(tokens):
    """扁平的 `:root{--wf-*}`，只含 token，不含任何線框機制的規則。"""
    decls = []
    for family, entries in (tokens or {}).items():
        if family == 'preset' or not isinstance(entries, dict):
            continue
        for name, entry in entries.items():
            decls.append(f'  {wf._theme_var_name(family, name)}: {_token_css_value(_token_raw(entry))};')
    return '/* generated by wfexport.py tokens --format css, do not edit */\n:root {\n' + \
           '\n'.join(decls) + '\n}'


def _tokens_as_ts(tokens):
    """扁平化成 TS 常數（別名已解析），供正式程式碼直接引用。"""
    out = {}
    for family, entries in (tokens or {}).items():
        if family == 'preset' or not isinstance(entries, dict):
            continue
        for name, entry in entries.items():
            raw = _token_raw(entry)
            if isinstance(raw, (list, tuple)):
                out[f'{family}.{name}'] = list(raw)
            else:
                text = str(raw)
                out[f'{family}.{name}'] = wf._THEME_FLATVALS.get(f'{family}.{name}', text) \
                    if text.startswith('{') else raw
    lines = ['// generated by wfexport.py tokens --format ts, do not edit',
             'export const tokens = {']
    for path in sorted(out):
        lines.append(f'  {json.dumps(path)}: {json.dumps(out[path], ensure_ascii=False)},')
    lines += ['} as const;', '', 'export type TokenName = keyof typeof tokens;', '']
    return '\n'.join(lines)


def _tokens_export(theme_path, fmt, kit_path=None):
    """`tokens`：把 theme 的 tokens 匯出成 DTCG JSON / CSS / TS。

    theme 若為 kit 元件（例如 canvas）寫了 components 規則，
    驗證時需要同一份 kit 才認得那些型別 —— 與 `lint --mockup --kit` 一致。
    """
    if kit_path:
        wf._load_kit(kit_path, explicit=True)
    wf._load_theme(theme_path)                       # 沿用既有驗證（未定義 ref、循環在此炸）
    tokens = wf._THEME_TOKENS
    if fmt == 'css':
        print(_tokens_as_css(tokens))
        return
    if fmt == 'ts':
        print(_tokens_as_ts(tokens))
        return
    dtcg, skipped = _tokens_to_dtcg(tokens)
    print(json.dumps(dtcg, ensure_ascii=False, indent=2))
    for path, why in skipped:
        print(f'warning: tokens.{path} 未匯出 —— {why}', file=sys.stderr)


# ── 匯入：DTCG .tokens.json → theme 的書寫形式 ─────────────────────────────
# 反向對照同一張表。目的是讓 Figma Variables 等工具匯出的 token 能直接變成 theme，
# 而不是逼作者去讀 $value / $type。
def _from_dimension(val, zero_bare=False):
    """{'value': 14, 'unit': 'px'} → '14px'；零值在複合字串裡可省略單位（CSS 慣例）。"""
    if isinstance(val, dict) and 'value' in val and 'unit' in val:
        num = val['value']
        num = int(num) if isinstance(num, float) and num == int(num) else num
        if zero_bare and num == 0:
            return '0'
        return f"{num}{val['unit']}"
    return None


def _from_dtcg_value(kind, val):
    """DTCG $value → theme 的一行寫法；無法還原時回 None。"""
    if isinstance(val, str) and val.startswith('{') and val.endswith('}'):
        return val                                   # 別名原樣保留
    if kind in ('dimension', 'duration'):
        return _from_dimension(val)
    if kind in ('color', 'fontFamily', 'fontWeight', 'number'):
        return val
    if kind == 'cubicBezier':
        return list(val) if isinstance(val, list) else None
    if kind == 'border' and isinstance(val, dict):
        width = _from_dimension(val.get('width'))
        style, color = val.get('style'), val.get('color')
        return f'{width} {style} {color}' if width and style and color else None
    if kind == 'shadow' and isinstance(val, dict):
        dims = [_from_dimension(val.get(k), zero_bare=True)
                for k in ('offsetX', 'offsetY', 'blur', 'spread')]
        if not all(dims[:3]) or not val.get('color'):
            return None
        parts = dims[:3] if dims[3] in (None, '0', '0px') else dims
        text = ' '.join(parts + [val['color']])
        return f'{text} inset' if val.get('inset') else text
    if kind == 'strokeStyle':
        if isinstance(val, str):
            return val
        if isinstance(val, dict) and isinstance(val.get('dashArray'), list):
            dims = [_from_dimension(d) for d in val['dashArray']]
            return ' '.join(d for d in dims if d) if all(dims) else None
    return None


def _dtcg_to_tokens(data, prefix=()):
    """DTCG dict → ({family: {name: 值}}, 略過清單)。只支援兩層（family.name）。"""
    out, skipped = {}, []
    for key, node in (data or {}).items():
        if key.startswith('$'):
            continue
        if not isinstance(node, dict):
            skipped.append(('.'.join(prefix + (key,)), '不是 token 也不是群組'))
            continue
        if '$value' in node:
            path = prefix + (key,)
            if len(path) != 2:
                skipped.append(('.'.join(path), 'theme 只支援 family.name 兩層，請先攤平'))
                continue
            kind = node.get('$type') or data.get('$type')
            if kind in ('typography', 'gradient'):
                # 複合型別回不到一行寫法，保留 DTCG 形狀（theme 本來就吃 $type/$value）
                shape = {'$type': kind, '$value': node['$value']}
                ext = (node.get('$extensions') or {}).get('space.ggmant.wireframe-lofi') or {}
                if ext.get('angle'):
                    shape['angle'] = ext['angle']
                out.setdefault(path[0], {})[path[1]] = shape
                continue
            text = _from_dtcg_value(kind, node['$value'])
            if text is None:
                skipped.append(('.'.join(path), f'無法還原成一行寫法（$type={kind!r}）'))
                continue
            out.setdefault(path[0], {})[path[1]] = text
            continue
        nested, nested_skipped = _dtcg_to_tokens(
            {'$type': node.get('$type', data.get('$type')),
             **{k: v for k, v in node.items() if not k.startswith('$')}},
            prefix + (key,))
        for fam, entries in nested.items():
            out.setdefault(fam, {}).update(entries)
        skipped += nested_skipped
    return out, skipped


def _yaml_scalar(value):
    """輸出好讀的 YAML 純量：色碼與含特殊字元的字串加引號，數字與陣列原樣。"""
    if isinstance(value, list):
        nums = [int(v) if isinstance(v, float) and v == int(v) else v for v in value]
        return '[' + ', '.join(str(v) for v in nums) + ']'
    if isinstance(value, (int, float)):
        return str(value)
    text = str(value)
    if re.fullmatch(r'-?\d+(?:\.\d+)?(px|rem|ms|s)?', text):
        return text
    if "'" in text:                                  # 含單引號改用雙引號，不要疊成 ''
        return '"' + text.replace('\\', '\\\\').replace('"', '\\"') + '"'
    return "'" + text + "'"


def _tokens_import(json_path):
    """`import`：DTCG JSON → theme 的 tokens 區塊（維持好寫好讀的形式）。"""
    with open(json_path, encoding='utf-8') as fh:
        data = json.load(fh)
    tokens, skipped = _dtcg_to_tokens(data)
    print('# generated by wfexport.py import, 貼進 theme 的 tokens: 區塊')
    print('tokens:')
    for family in sorted(tokens):
        items = tokens[family]
        if any(isinstance(v, dict) for v in items.values()):
            print(f'  {family}:')
            for name, val in items.items():
                if isinstance(val, dict):
                    block = yaml.safe_dump({name: val}, allow_unicode=True, sort_keys=False, default_flow_style=None)
                    print(''.join('    ' + line + '\n' for line in block.rstrip().split('\n')), end='')
                else:
                    print(f'    {name}: {_yaml_scalar(val)}')
            continue
        entries = ', '.join(f'{name}: {_yaml_scalar(val)}' for name, val in items.items())
        print(f'  {family}: {{ {entries} }}')
    for path, why in skipped:
        print(f'warning: {path} 未匯入 —— {why}', file=sys.stderr)


def _argval(flag, argv):
    if flag in argv:
        i = argv.index(flag)
        if i + 1 < len(argv):
            return argv[i + 1]
    return None


def main(argv=None):
    argv = list(sys.argv[1:] if argv is None else argv)
    if not argv or argv[0] in ('-h', '--help') or argv[0] not in ('tokens', 'types', 'import'):
        print(__doc__.strip(), file=sys.stderr)
        return 1
    cmd, rest = argv[0], argv[1:]
    tmap = _argval('--types-map', rest)
    kit = _argval('--kit', rest)
    paths = [a for a in rest if not a.startswith('-') and a not in (tmap, kit)]
    fmt = (_argval('--format', rest) or ('json' if cmd == 'tokens' else 'ts')).lower()
    if fmt in paths:
        paths.remove(fmt)
    if not paths:
        print(f'usage: wfexport.py {cmd} <file.yaml> [--format ...]', file=sys.stderr)
        return 1
    try:
        if cmd == 'import':
            _tokens_import(paths[0])
        elif cmd == 'tokens':
            if fmt not in ('json', 'css', 'ts'):
                raise ValueError(f"tokens 的 --format 只接受 json / css / ts（收到 {fmt!r}）")
            _tokens_export(paths[0], fmt, kit)
        else:
            if fmt not in ('ts', 'json'):
                raise ValueError(f"types 的 --format 只接受 ts / json（收到 {fmt!r}）")
            _types_export(paths[0], fmt, tmap)
    except Exception as exc:                          # 作者錯誤印一行，不噴 traceback
        first, _, more = str(exc).partition('\n')
        print(f'error: {first}', file=sys.stderr)
        for line in more.splitlines():
            if line.strip():
                print(f'  {line}', file=sys.stderr)
        return 1
    return 0


if __name__ == '__main__':
    sys.exit(main())
